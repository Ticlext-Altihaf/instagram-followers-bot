...
class InstagramAPI:
    ...
    def __init__(self, username, password, debug=False, IGDataPath=None, proxy=False, quota=None):
        m = hashlib.md5()
    ...
    def consumeQuota(self, type):
        if self.quota is None: return True
        if type in self.quota:
            if self.quota[type] - 1 > 0:
                self.quota[type] -= 1
                return True
            else:
                print("Quota exceeded: " + type)
                return False
        print("Couldn't find quota info for %s" % type)
        return True
